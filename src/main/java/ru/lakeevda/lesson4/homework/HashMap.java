package ru.lakeevda.lesson4.homework;
...
//
    class HashMapIterator implements Iterator<HashMap.Entity>{
        ...
        private Bucket[] bucketsIter;

        private Integer cursor = 0;
        public HashMapIterator(Bucket[] buckets) {
            this.bucketsIter = new Bucket[size];
            int i = 0;
            for(Bucket buf : buckets) {
                if (buf != null) {
                    this.bucketsIter[i++] = buf;
                }
            }
        }
        ...
        public boolean hasNext() {
            return cursor < bucketsIter.length;
        }

        @Override
        public Entity next() {
            return bucketsIter[cursor++].head.value;
        }
    }
    ...
    public String toString() {
        Iterator<HashMap.Entity> iterator = iterator();
        String result = "";
        while (iterator.hasNext()) {
            Entity entity = iterator.next();
            if (result.equals("")) result = "[";
            else result = result + ", ";
            result = result + entity.key + "=" + entity.value;
        }
        return result + "]";
    }
